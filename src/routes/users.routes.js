...
import { safeUser, hashPassword } from "../lib/helpers.js";
...
router.post("/", allowRoles("Administrateur"), async (req, res, next) => {
  try {
    const { motDePasse, ...data } = req.body;
    if (!data.nom || !data.prenom || !data.email || !motDePasse) {
...
router.patch("/:id/password", authenticate, async (req, res, next) => {
  try {
    ...
    const updated = await prisma.utilisateur.update({
      where: { id: req.params.id },
      data: { motDePasse: await hashPassword(newPassword) }
    });

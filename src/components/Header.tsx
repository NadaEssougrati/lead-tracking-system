    ...
    <header className="bg-white border-b border-slate-100 text-slate-800 h-16 flex items-center justify-between px-6 sticky top-0 z-40 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-100" id="app-header">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative">
          ...
          {showNotifDropdown && (
            <div className="absolute right-0 mt-3 w-80 bg-white rounded-lg shadow-xl border border-slate-200 overflow-hidden z-50 dark:bg-slate-900 dark:border-slate-800">
                  ...
                  notifications.map((notif) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-slate-800 font-semibold dark:text-white">{notif.titre}</p>
                        <p className="text-[11px] text-slate-500 truncate mt-0.5 dark:text-slate-400">{notif.message}</p>
                        <p className="text-[9px] text-slate-400 mt-1 dark:text-slate-500">
        ...
        <div className="flex items-center gap-2.5 pl-2 border-l border-slate-100 dark:border-slate-800">
          <div className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-700 border border-slate-200 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-350">
            <UserRound className="h-4 w-4" />
          </div>
          <div className="hidden lg:block text-left">
            <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">{activeUser.nom}</p>
            <p className="text-[10px] text-slate-500 font-medium dark:text-slate-400">{activeUser.role}</p>
          </div>
          <button
